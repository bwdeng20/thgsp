...
class TestIbFix:
    def test_bipartite(self):
        ...
        for i in range(n_sample):
            ...
            flag, vtx_color, _ = is_bipartite_fix(B.to_scipy("csr"), fix_flag=False)
            assert flag
            NB = rand_udg(N, 0.8)  # complete graph must be non-bipartite
            flag, vtx_color, _ = is_bipartite_fix(NB.to_scipy("csr"), fix_flag=False)
            assert not flag

    def test_bipartite_fix(self):
        N = 12
        B = rand_udg(N, 0.3)
        flag, vtx_color, Bb = is_bipartite_fix(B.to_scipy("csr"), fix_flag=True)
