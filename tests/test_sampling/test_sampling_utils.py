...
from thgsp.sampling._utils import construct_sampling_matrix, construct_hth, construct_dia
...
def test_construct_sampling_matrix(dtype, dv, layout):
    ...
    HtHD1 = construct_hth(N, S, xp.arange(1, N + 1)[S], dtype=dtype, device=dv, layout=layout)
    # print(HtH1.A)
    # print(HtHD1.A)
...
def test_construct_dia(dt, dv, layout, inverse, narrow):
    ...
    P = construct_dia(S, diag_data=diag_data, ps=narrow, inverse=inverse, dtype=dt, layout=layout)
    if narrow:
        assert P.shape == (M, M)
        if inverse:
            assert xp.allclose(P.diagonal(), 1 / diag_data[S], )
        else:
